...
async def run_gemini_gold_trader_loop() -> None:
    ...
    (
        ...
        get_chart_klines("1m", cfg.chart_bars_1m, user_id=cfg.demo_user_id),
        get_chart_klines("5m", cfg.chart_bars, user_id=cfg.demo_user_id),
    ...
    if not klines_ready(bars_5m, bars_15m, bars_1h):
        logger.info(
            ...
            "15m=%s/%s source=%s, 1h=%s/%s source=%s)",
            len(bars_1m),
            meta_1m.get("status"),
            meta_1m.get("source"),
            len(bars_5m),
