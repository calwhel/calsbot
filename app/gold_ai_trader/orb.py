...
from app.gold_ai_trader.models import GoldAiOrbState
...
def _ensure_state(db, *, now: datetime, session: str, cfg) -> GoldAiOrbState:
    trade_day = now.date()
    row = (
        db.query(GoldAiOrbState)
        .filter(
            GoldAiOrbState.trade_day_utc == trade_day,
            GoldAiOrbState.session == session,
        )
        .first()
    )
...
def _persist_state(db, row: GoldAiOrbState) -> None:
    db.add(row)
    db.commit()
    db.refresh(row)
...
async def detect_orb_signal(
    *,
...
) -> Tuple[Optional[OrbSignal], Optional[GoldAiOrbState], str]:
    """
    ...
    k = await get_gold_ai_klines(tf, 160, user_id=user_id) or []
    ...
    k5 = await get_gold_ai_klines("5m", 80, user_id=user_id) or []
