    ...
    async def analyze_momentum(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if bullish_5m and bullish_15m:
                ...
                # OVEREXTENDED SHORT CONDITIONS (SLIGHTLY STRICTER):
                # 1. RSI 58+ (overbought)
                # 2. Volume 1.2x+ (volume confirms the move)
                ...
                is_overextended_short = (
                    rsi_5m >= 58 and  # Slightly stricter (was 55)
                    volume_ratio >= 1.2 and  # Slightly stricter (was 1.0x)
                    price_to_ema9_dist >= 2.5  # Slightly stricter (was 2.0%)
                )
                ...
                # SKIP: Not overextended enough for SHORT, not exceptional enough for LONG
                    ...
                    logger.info(f"{symbol} Still pumping but NOT overextended yet: Vol {volume_ratio:.1f}x, RSI {rsi_5m:.0f}, Distance {price_to_ema9_dist:+.1f}% (need RSI 58+, Vol 1.2x+, Distance 2.5%+)")
                    return None
            ...
            # ═══════════════════════════════════════════════════════
                ...
                is_strong_dump = (
                    ...
                    current_candle_size >= 1.0 and  # Slightly stricter (was 0.8%)
                    volume_ratio >= 1.4 and  # Slightly stricter (was 1.2x)
                    38 <= rsi_5m <= 68  # Slightly stricter (was 35-70)
                )
                ...
                # Resumption entry: Slightly stricter
                if has_resumption_pattern and rsi_5m >= 48 and rsi_5m <= 68 and volume_ratio >= 1.1:
                    return {
                        'direction': 'SHORT',
                        'confidence': 95,
                        'entry_price': current_price,
                ...
                # SKIP - no valid entry pattern
                    ...
                    if rsi_5m < 38 or rsi_5m > 68:
                        skip_reason.append(f"RSI {rsi_5m:.0f} out of range (need 38-68)")
                    if volume_ratio < 1.1:
                        skip_reason.append(f"Low volume {volume_ratio:.1f}x (need 1.1x+)")
            ...
            # ═══════════════════════════════════════════════════════
                ...
                good_rsi = rsi_5m >= 57  # Slightly stricter (was 55)
                ...
                if has_exhaustion_signs:
                    ...
                    if not good_rsi:
                        skip_reason.append(f"RSI {rsi_5m:.0f} (need 57+)")
            ...
            # ═══════════════════════════════════════════════════════
                ...
                # Check for early reversal pattern - Slightly stricter
                is_early_reversal = (
                    current_candle_bearish and  # Current candle is red
                    bearish_momentum and  # Recent momentum turning down
                    rsi_5m >= 52 and rsi_5m <= 68 and  # Slightly stricter (was 50-70)
                    volume_ratio >= 1.3  # Slightly stricter (was 1.2x)
                )
