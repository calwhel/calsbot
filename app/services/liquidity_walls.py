...
async def _ai_summary(report: dict, symbol: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            "RULES: You MUST always give an actionable TRADE — never 'wait', never 'no setup', "
            "never 'flat'. If there is no clean market entry right now, give a LIMIT order at the "
            "best wall/level (e.g. 'LONG @ 64,820 [LIMIT — buy at the FRESH BIG buy wall]'). "
            "Always pick the higher-probability side based on pressure + wall behavior, even if "
            "edge is thin — say so in NOTE.\n\n"
            ...
            "TRADE: <LONG or SHORT> @ <price> [MARKET] or [LIMIT — short reason]\n"
            "STOP: <price> (<distance %>)\n"
            "TP: <price> (<distance %>)\n"
