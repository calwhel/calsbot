...
async def cmd_test_subscription(message: types.Message):
    """Admin command to test subscription notification"""
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.telegram_id == str(message.from_user.id)).first()
        if not user or not user.is_admin:
        ...
        test_user = db.query(User).filter(User.telegram_id != str(message.from_user.id)).first()
        if not test_user:
            await message.answer("❌ No other user found to test with.")
            return
        
        # Get all admins
        admins = db.query(User).filter(User.is_admin == True).all()
        
        user_info = f"@{test_user.username}" if test_user.username else f"{test_user.first_name} (ID: {test_user.telegram_id})"
        plan_name = "🤖 Auto-Trading"
        referred_info = ""
        if test_user.referred_by:
            referrer = db.query(User).filter(User.referral_code == test_user.referred_by).first()
            if referrer:
        ...
        for admin in admins:
            try:
                ...
                logger.error(f"Failed to send test notification to admin {admin.telegram_id}: {e}")
        
        await message.answer(f"✅ Test subscription notifications sent to {sent_count} admin(s)!")
    except Exception as e:
...
@dp.message(Command("test_referral"))
async def cmd_test_referral(message: types.Message):
    """Admin command to test referral notification"""
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.telegram_id == str(message.from_user.id)).first()
        if not user or not user.is_admin:
        ...
        referrer = db.query(User).filter(User.referral_code.isnot(None)).first()
        if not referrer:
            await message.answer("❌ No user with referral code found.")
            return
        
        test_subscriber = db.query(User).filter(User.telegram_id != referrer.telegram_id).first()
        if not test_subscriber:
            await message.answer("❌ Could not find test subscriber.")
            return
        
        ref_username = f"@{referrer.username}" if referrer.username else referrer.first_name
        new_sub_username = f"@{test_subscriber.username}" if test_subscriber.username else test_subscriber.first_name
        ...
        for admin in admins:
            try:
                await bot.send_message(
                    admin.telegram_id,
                    f"🎁 <b>NEW REFERRAL PAYOUT PENDING!</b> (TEST)\n\n"
        ...
        await message.answer(f"✅ Test referral notifications sent to {sent_count} admin(s)!")
    except Exception as e:
        await message.answer(f"❌ Error: {e}")
    finally:
        db.close()
