...
- **Parabolic Strategy (AI-POWERED Jan 2026)**: AI-validated short entries on exhausted 50%+ pumps. Pre-filters candidates based on RSI ≥75, EMA overextension (>1.5%), wick rejection (≥1%), and volume surge (≥1.5x). AI validates each candidate with reversal confidence score and sets dynamic TP/SL (typically 5-8% TP, 3-5% SL at 20x leverage). Only A/A+ quality setups are approved. AI receives exhaustion data, BTC correlation, and market context.
- **LONGS Strategy (AI-POWERED Jan 2026)**: Enhanced AI-driven signal generation using GPT-4o-mini with sophisticated decision framework. Pre-filters coins 5-50% 24h range with liquidity/manipulation checks. AI receives: RSI, EMA structure (9>21 bullish), volume ratio, funding rate, BTC correlation, last 3 candles pattern, and volatility metrics. AI decides with entry quality grades (A+/A/B/C) - only A+ and A grades approved. Dynamic TP/SL based on volatility (typically 2.5-5% TP, 2-4% SL at 20x). Requires confidence 6+/10 for approval. AI provides plain English reasoning and R:R ratio with each signal.
- **AI Signal Validation (Jan 2026)**: Both LONG and SHORT strategies use AI validation with retry logic (4 retries, exponential backoff for rate limits). AI evaluates multiple technical factors including trend structure, momentum, volume, and market context. Signals display AI quality grade, confidence score, and reasoning. Rejected signals are logged for analysis.
