...
- **Built-in Backtester**: Integrated into wizard Step 7. Users click "Quick Test (30d)" or "Deep Test (90d)" to replay their strategy against real 1h OHLCV data from Kraken (BTC/ETH/SOL/PEPE/40+ coins). Results show win rate, total P&L, profit factor, max drawdown, avg hold time, equity curve (inline SVG), and a full trade log. Available to all users. Endpoint: `POST /api/backtest/run`. Engine: `app/services/backtest_engine.py`.
