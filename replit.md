...
- **LONGS Strategy (AI-POWERED Jan 2026)**: Fully AI-driven signal generation using GPT-4o-mini (same approach as successful scan feature). Pre-filters coins 5-50% 24h range with liquidity/manipulation checks, then passes technical data to AI for analysis. AI receives: RSI, EMA structure, volume ratio, funding rate, BTC correlation, last 3 candles pattern. AI decides STRONG BUY/BUY/HOLD/AVOID with confidence score (7+/10 required for approval). Single TP at +67% with -65% SL @ 20x leverage (3.35% price move for TP, 3.25% for SL). AI reasoning included in signal messages.
- **AI Signal Filter (Jan 2026)**: Uses GPT to validate every signal before broadcast. AI analyzes technical data, risk factors, BTC correlation, and entry timing. Signals must pass AI approval (confidence 5+) to be broadcast. Rejected signals are logged with reasoning. AI analysis included in signal messages showing recommendation (STRONG BUY/BUY/HOLD/AVOID), confidence score, and plain English explanation.
