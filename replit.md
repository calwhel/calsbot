...
## Recent Changes
- **Dashboard UI Improvements** (October 2025): Complete dashboard reorganization for better UX. Live exchange balance now displays prominently (shows real-time Bitunix USDT balance when connected). Paper trading moved to compact section instead of dominating the dashboard. Live account section shows balance + today's PnL with color-coded indicators. Cleaner, more professional layout prioritizing live trading information.
- **Bitunix Leverage Safety Fix** (October 2025): **CRITICAL SAFETY FIX** - Fixed dangerous leverage bug where trades were executing with wrong leverage (20x instead of configured 10x). Added `set_leverage()` method that calls `/api/v1/futures/account/change_leverage` endpoint BEFORE placing orders. This prevents unexpected liquidations and ensures trades execute with user-configured leverage. All Bitunix trades now properly set leverage on the exchange before order placement.
- **Bitunix Authentication Fix** (October 2025): Fixed critical authentication bug where Bitunix API was returning "System error" (code 2). Implemented complete fix with: (1) Double SHA256 signature: `SHA256(SHA256(nonce + timestamp + api-key + queryParams + body) + secretKey)`, (2) Correct timestamp format (YmdHis: `20241120123045` instead of milliseconds), (3) Correct endpoint (`/api/v1/futures/account` with `marginCoin=USDT` parameter), (4) Proper query param formatting for signature (`marginCoinUSDT` concatenated format), (5) 32-character hex nonce generation, (6) Integer error code handling.
