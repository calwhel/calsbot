...
- **Signal Generator**: Fetches OHLCV data via CCXT, performs multi-timeframe (1h, 4h) EMA crossover analysis with relaxed volume confirmation (≥80% of average), RSI filtering (75/25 thresholds), ATR-based dynamic stops, and support/resistance identification. Includes trend-following signals for strong trends (0.5%+ EMA separation) without requiring crossovers. Risk scoring accepts LOW/MEDIUM signals, only rejecting extreme HIGH risk (score ≥4). Take profit targets set at realistic levels (1R, 1.5R, 2R). **No signal cooldowns** - autotrader adapts instantly to changing market conditions (news, momentum shifts, spot flows).
