...
- **Hybrid Data Source Architecture**: Uses Binance Futures for technical analysis data with automatic Bitunix fallback. When Binance lacks candle data for Bitunix-exclusive coins (common for new listings), the system automatically fetches candles from Bitunix API. Intelligent fallback logic only attempts Bitunix when Binance returns insufficient data for the requested limit (not just an arbitrary threshold). System gracefully handles Bitunix klines API errors (code 2 "System error") and falls back to Binance-only data when available. (Nov 25, 2025: Fixed signal drought by lowering minimum volume from $400K→$150K→$50K for Bitunix thin books - even 100%+ pumpers typically have <$80K volume on Bitunix! Also: lowered candle requirements from 30→15, expanded freshness windows, and lowered volume ratios to 1.2x. AGGRESSIVE SHORTS MODE: Minimum pump 20%+, RSI 55+, price extension 2.0%+.)
