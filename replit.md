...
- **AI-POWERED LONGS Strategy (Jan 2026 - ULTRA STRICT)**: Only targets coins +2% to +12% on the day (early momentum). TA pre-filters require 5/6 confirmations before AI validation: (1) Liquidity $5M+, (2) Anti-manipulation ≤18%, (3) Trend: BOTH 5m AND 15m bullish with EMA spread ≥0.30%, (4) RSI 40-62 (strict - avoid overbought), (5) Volume ≥1.5x (strict), (6) Price position <65% (strict - don't buy highs). Additional filters: (7) BTC must be stable/bullish (>-1.5%), (8) Last 5m candle must be GREEN (no fading momentum). AI validation requires A+/A quality only with confidence ≥8. Prompt explicitly tells AI to be STRICT and reject marginal setups. Rate limited: 5s between calls.
