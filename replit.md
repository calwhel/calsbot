...
- **NumPy Type Conversion Fix** (Oct 24): Fixed PostgreSQL error where NumPy types (np.float64) were being passed to database causing "schema 'np' does not exist" error. All signal generators (reversal_scanner, hybrid_signals, signals) now convert pandas/NumPy types to native Python floats before database insertion.
- **Signal Broadcasting Fix** (Oct 24): Fixed two critical bugs preventing signals from broadcasting: (1) Funding rate symbol format - now uses perpetual futures format (BTC/USDT:USDT) instead of spot format, (2) Signal model field cleanup - removes non-database fields (pattern, category objects, quality metrics) before saving. Bot was detecting signals but crashing before broadcast.
- **Leverage-Aware TP/SL Fix** (Oct 23): Fixed critical bug where TP/SL calculations didn't account for 10x leverage. Percentages are now properly converted to price movements: SWING 20% SL = 2% price movement, 15%/30%/50% TPs = 1.5%/3%/5% price movements. SCALP 12% SL = 1.2% price movement, 10%/15%/20% TPs = 1%/1.5%/2% price movements. Prevents absurd targets like $120k on $100k BTC.
- **TP/SL Calculation Fix** (Oct 23): Fixed critical bug where signals.py and reversal_scanner.py were using outdated TP/SL values (25% SL, 20%/40%/60% TPs). All signal generators now use correct hybrid system percentages: SWING signals have 20% SL with 15%/30%/50% TPs, SCALP signals have 12% SL with 10%/15%/20% TPs.
- **100% Binance Market Data** (Oct 23): All market data now sourced from Binance (price feeds, funding rates, technical indicators, multi-timeframe analysis) for maximum reliability and consistency. Bitunix remains exclusive execution venue.
...
- **Smart Exit System** (Oct 22, Fixed Oct 24): AI-powered reversal detection closes trades early when market turns against your position. Monitors 6 reversal signals: (1) EMA death/golden crosses, (2) Bearish/bullish engulfing candles, (3) RSI divergence (price vs momentum), (4) Extreme RSI zones with rejection candles, (5) Volume spike distribution/accumulation, (6) Profit protection (15%+ leveraged profit with BOTH trending EMAs + extreme RSI). Fixed overly aggressive exit that was closing winning trades during healthy pullbacks - now requires 3x higher profit threshold (15% vs 5%) and stricter reversal confirmation (RSI <45/>55 + trending EMAs, not just neutral territory).
...
- **Trading Style Changed to SWING TRADING** (Oct 21): Switched from scalping to swing trading with percentage-based targets. SL: 20%, TP1: 15%, TP2: 30%, TP3: 50% from entry. Trades hold for bigger moves instead of quick scalps.
...
- **Risk Management**: Percentage-based SL/TP (SCALP: 12% SL, 10%/15%/20% TPs; SWING: 20% SL, 15%/30%/50% TPs), risk-based position sizing, daily loss limits, and max drawdown protection.
